"""
Patched goal.py - Functions for specifying goals and reward calculations.

This version doesn't require spacy - uses simpler string matching instead.
"""
...
def get_human_goals(all_products, product_prices):
    ...
    for goal in goals:
        goal["weight"] = 1
    rprint(f"[yellow]{cnt} goals skipped (no attributes)[/yellow]")
    return goals
...
def get_synthetic_goals(all_products, product_prices):
    ...
    for goal in goals:
        goal["weight"] = (
            sum(1.0 / cnt_atts[att] for att in goal["attributes"])
            / len(goal["attributes"])
        )
    return goals
...
def _extract_nouns(text: str) -> list[str]:
    """
    Simple noun extraction without spacy.

    Extracts words that are likely nouns based on simple heuristics:
    ...
    # Simple heuristic: return all significant words (length > 2)
    ...
    stop_words = {
        ...
        "there", "then", "once", "find", "me", "looking", "want", "need",
        "please", "get", "buy", "purchase", "item", "product",
    }
...
def get_type_reward(purchased_product, goal):
    ...
    This is a simplified version that doesn't require spacy.
    """
    ...
    # Use simple noun extraction instead of spacy
    purchased_type_parse = _extract_nouns(purchased_type)
    desired_type_parse = _extract_nouns(desired_type)
